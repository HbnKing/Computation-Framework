 ...
 *
 */
public class StormTopologyCount {
	...
	public static class MySpout extends BaseRichSpout{
		...
		public void nextTuple() {
			...
			this.collector.emit(new Values(num));//这里需要一个list 集合   Values继承list//把num封装到一个tuple里面，发射出去
			Utils.sleep(1000);
		}
	...
	public static class MyBolt extends BaseRichBolt{
		...
		public void execute(Tuple input) {
			//input.getInteger(0);  //通过list的角标获取值  
	...
	public static void main(String[] args) {
		...
		topologyBuilder.setBolt("bolt1", new MyBolt()).shuffleGrouping("spout1"); //指定 bolt1读取的数据
		
		StormTopology createTopology = topologyBuilder.createTopology();
		String topologyName = StormTopologyCount.class.getSimpleName();
		Config config = new Config();
		if(args.length==0){//根绝参数判断是否在本地执行
			//创建一个本地集群运行程序
